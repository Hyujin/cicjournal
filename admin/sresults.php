<?php
  ...
  $query = "SELECT * FROM post WHERE title LIKE '%$_GET[search_id]%'";
  $number_per_page = $paginate->getNumberPerPage();
  $current_page_num = (isset($_GET['page']) ? $_GET['page'] : 1);
  $number_of_result = mysqli_num_rows($paginate->getNumberOfResult($query));
  $total_pages = $paginate->getTotalPages();
  $starting_limit = ($current_page_num - 1) * $number_per_page;
  $query .= " LIMIT $starting_limit,$number_per_page";
  $posts_per_page = $paginate->getPostsPerPage($query);

  if (isset($_GET['sort']) && isset($_GET['search_id'])) {
    $id = $paginate->sanitize($_GET['search_id']);

    if ($_GET['sort'] == 'newest') {
      $query = "SELECT * FROM post WHERE title LIKE '%$id%' ORDER BY date_posted DESC ";
      $number_of_result = mysqli_num_rows($paginate->getNumberOfResult($query));
      $total_pages = $paginate->getTotalPages();
      $starting_limit = ($current_page_num - 1) * $number_per_page;
      $query .= "LIMIT $starting_limit,$number_per_page";
      ...
      $query = "SELECT * FROM post WHERE title LIKE '%$id%' ORDER BY date_posted ASC ";
      $number_of_result = mysqli_num_rows($paginate->getNumberOfResult($query));
      $total_pages = $paginate->getTotalPages();
      $starting_limit = ($current_page_num - 1) * $number_per_page;
      $query .= "LIMIT $starting_limit,$number_per_page";
      ...
      $query = "SELECT * FROM post WHERE title LIKE '%$id%' AND status = 'INACTIVE'";
      $number_of_result = mysqli_num_rows($paginate->getNumberOfResult($query));
      $total_pages = $paginate->getTotalPages();
      $starting_limit = ($current_page_num - 1) * $number_per_page;
      $query .= "LIMIT $starting_limit,$number_per_page";
      ...
      $query = "SELECT * FROM post WHERE title LIKE '%$id%' AND status = 'ACTIVE'";
      $number_of_result = mysqli_num_rows($paginate->getNumberOfResult($query));
      $total_pages = $paginate->getTotalPages();
      $starting_limit = ($current_page_num - 1) * $number_per_page;
      $query .= "LIMIT $starting_limit,$number_per_page";
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                  <div class="h1 mb-0 text-gray-800 mb-5">Search Result/s</div>
                    <!-- Page Heading -->
                     ...
                     <ul class="pagination pg-blue" >
                       <?php
                         if ($current_page_num == 1){
                           ...
                           $href = ( isset($_GET['sort']) ? "sresults.php?search_id=". $_GET['search_id'] ."&sort=". $_GET['sort']. "&page=". ($current_page_num-1) : "sresults.php?search_id=".$_GET['search_id']."&page=".($current_page_num-1));
                           echo "
                           <li class='page-item '>
                       ...
                       <?php
                         for ($i=0; $i < $total_pages; $i++) {
                           $href = ( isset($_GET['sort']) ? "sresults.php?search_id=". $_GET['search_id'] ."&sort=". $_GET['sort']. "&page=". ($i+1) : "sresults.php?search_id=".$_GET['search_id']."&page=".($i+1));
                                ...
                                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                                    ...
                                    <div class="dropdown no-arrow">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right shadow animated--fade-in"
                                            ...
                                            <a class="dropdown-item" href='sresults.php?search_id=<?php echo $_GET['search_id'] ?>&sort=newest&page=1'>Newest to oldest</a>
                                            <a class="dropdown-item" href='sresults.php?search_id=<?php echo $_GET['search_id'] ?>&sort=oldest&page=1'>Oldest to newest</a>
                                            <a class="dropdown-item" href='sresults.php?search_id=<?php echo $_GET['search_id'] ?>&sort=hidden&page=1'>Hidden</a>
                                            <a class="dropdown-item" href='sresults.php?search_id=<?php echo $_GET['search_id'] ?>&sort=visible&page=1'>Visible</a>
                                        </div>
                                    </div>
                                </div>
    ...
    echo "<script>
      $(document).ready(function(){
        ...
        $('#modal-success-close').on('click', function(event){
          window.location.href = 'deletepap.php?post_id=".$_POST['post_id']."';
        });
